...
def swf(self: BaseHTTPRequestHandler, query, path):
    ...
    if self.no_cache or not cache_path.exists():
        if self.url.netloc:
            remote_url = self.url.geturl()
        else:
            remote_url = self.base_url + path
        logging.info(f"Downloading at {remote_url}")
        retry = 3
        while True:
            try:
...
@handle("get", "", r".+")
def unhandled_get(self, query):
    logging.info(f"Swallowing unhandled GET {self.url.geturl()}")
    self.send_response(200)
...
@handle("get", "save.api.4399.com", r"/")
def save_time(self, query):
    logging.info(f"GET /save time, {query=}")
    self.send_response(200)
    self.send_header("Content-Type", "application/json; charset=utf-8")
    self.end_headers()
    self.wfile.write(
        json.dumps({"time": time.strftime("%Y-%m-%d %H:%M:%S")}).encode()
    )
...
@handle("post", "game-audit.4399api.net", r"/v1\.0/.+/check/text\.json")
def audit_text(self, data):
    logging.info(f"Audit text check {self.url.geturl()}")
    self.send_response(200)
    self.send_header("Content-Type", "application/json; charset=utf-8")
    self.end_headers()
    self.wfile.write(json.dumps({"result": {"riskType": 0}}).encode())


@handle("post", "", r".+")
def unhandled_post(self, data):
    logging.info(f"Swallowing unhandled POST {self.url.geturl()}")
    self.send_response(200)
    self.send_header("Content-Type", "application/json; charset=utf-8")
    self.end_headers()
    self.wfile.write(b"{}")
...
class SpeculumHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        handlers = HANDLERS["get"].get(self.url.netloc, []) + HANDLERS["get"].get(
            "", []
        )
    ...
    def do_POST(self):
        ...
        handlers = HANDLERS["post"].get(self.url.netloc, []) + HANDLERS["post"].get(
            "", []
        )
        for handler in handlers:
            match = handler["pattern"].match(self.url.path)
